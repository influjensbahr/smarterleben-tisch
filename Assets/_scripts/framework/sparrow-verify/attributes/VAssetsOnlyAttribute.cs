...
namespace Sparrow.Verification
{
    ...
    public class VAssetsOnlyAttribute : Attribute, IVerifyAttribute
    {
    #if UNITY_EDITOR
        ...
        private static void CheckUnityObject(VerifyCheckBase checker, UnityEngine.Object unityObject, FieldInfo field, UnityEngine.Object parentObject)
        {
            if (field.FieldType.IsSubclassOf(typeof(UnityEngine.Object)))
            {
                if (unityObject == null)
                {
                    checker.Check(false, $"VAssetsOnly: {field.Name} is not an assigned",
                        parentObject,
                        category: "VAssetsOnly Attribute");
                }
                else
                {
                    string assetPath = AssetDatabase.GetAssetPath((UnityEngine.Object)unityObject);
                    bool isAsset = !string.IsNullOrEmpty(assetPath);

                    checker.Check(isAsset, $"VAssetsOnly: {field.Name} is not an asset in the project",
                        parentObject,
                        category: "VAssetsOnly Attribute");
                }
            }
