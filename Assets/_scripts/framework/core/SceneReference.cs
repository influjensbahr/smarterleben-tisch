// from https://github.com/NibbleByte/UnitySceneReference/blob/master/Assets/DevLocker/Utils/SceneReference.cs
...
#pragma warning restore 0414
        ...
        public void AddToBuild()
        {
            var newScenes = new EditorBuildSettingsScene[] {
                        new EditorBuildSettingsScene(AssetDatabase.GetAssetPath(m_SceneAsset), true)
                    };
        ...
        public bool IsInBuildSettings()
        {
            ...
            {
                long localId;
                if (AssetDatabase.TryGetGUIDAndLocalFileIdentifier(m_SceneAsset, out string guid, out localId))
                {
                    indexInSettings = Array.FindIndex(EditorBuildSettings.scenes, s => s.guid.ToString() == guid);
                }
            }

            return indexInSettings != -1;
        }
